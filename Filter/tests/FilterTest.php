...
class FilterTest extends TestCase
{
    ...
    {
        // Ожидаемое AST дерево для выражения like(Brand, "Mazda")
        $expect = new T\LikeExpr(new T\FldVal('1'), new T\StrVal('0'));

        // Проверяем, что выражение разбирается корректно
        $ast = $this->filter->getAst('like(1, 0)');
        $this->assertEquals($expect, $ast, 'Не удалось корректно разобрать выражение like');

        // Проверка метода apply
        $this->assertTrue($expect->apply(['Brand' => 'Mazda']), 'Метод apply не сработал для совпадающей строки');
        $this->assertTrue($expect->apply(['Brand' => 'Mazda MX-5']), 'Метод apply не сработал для совпадающего шаблона');
        $this->assertFalse($expect->apply(['Brand' => 'Toyota']), 'Метод apply сработал для несовпадающей строки');
    }
